...
| 계산 엔진 | `manseryeok` (npm) | 서버에서 실행 (검증 완료·채택, `saju-engine.md`) |
...
│                                               │
│   ① manseryeok 실행 → 여덟 글자·오행 계산   │ ← 계산(결정론)
│                                               │
│   ② 계산 결과 + 해석 프롬프트                  │
...
- **서버 전용 실행**: `manseryeok`은 Node 패키지라 클라이언트에서 실행 불가. Gemini 호출도 키 보안상 서버에서만.
...
2. **음력 입력**: `manseryeok`이 KASI 정본 데이터로 직접 변환(별도 라이브러리 불필요), 윤달 플래그 그대로 전달. (상세 `saju-engine.md`)
3. **사주 계산**: `manseryeok` 실행 → 여덟 글자·오행·십신·신강신약·용신 등 산출.
